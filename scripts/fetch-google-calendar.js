...
function loadCredentials() {
  ...
  if (GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET) {
    const redirect_uris = GOOGLE_REDIRECT_URIS
      ? GOOGLE_REDIRECT_URIS.split(",")
  ...
  if (!fs.existsSync(CREDENTIALS_PATH)) {
    console.error(
      "Missing credentials.json in project root and no GOOGLE_* env vars found. See README for setup."
    );
    process.exit(1);
  }
...
async function authorize() {
  ...
  const AUTO_OAUTH =
    process.env.AUTO_OAUTH === "1" || process.env.AUTO_OAUTH === "true";
  const localRedirect = "http://localhost:51234/oauth2callback";
  const chosenRedirect = AUTO_OAUTH
    ? localRedirect
  ...
  if (AUTO_OAUTH) {
    ...
    console.log(
      "Starting local server to capture OAuth callback at",
      localRedirect
    );
    const server = http.createServer(async (req, res) => {
      try {
        ...
        if (code) {
          ...
          res.end(
            "<html><body><h1>Authorized</h1><p>You can close this window.</p></body></html>"
          );
          // exchange code
    ...
    if (open) {
      try {
        ...
        console.log("Failed to open browser, visit URL manually:", authUrl);
      }
    } else {
...
async function fetchAllEvents(auth) {
  ...
  do {
    ...
    items.forEach((c) =>
      calendars.push({ id: c.id, summary: c.summary || c.id })
    );
  ...
  fs.writeFileSync(
    OUT_PATH,
    JSON.stringify({ fetchedAt: new Date().toISOString(), data: out }, null, 2),
    "utf8"
  );
...
async function main() {
  try {
    ...
    if (icalUrl) {
      console.log(
        "Found GOOGLE_ICAL_URL, fetching public ICS feed for test..."
      );
      const ical = require("node-ical");
      const axios = require("axios");
      let res;
      try {
        res = await axios.get(icalUrl, {
      ...
      if (/<html|<!doctype|<body/i.test(body)) {
        console.log(
          "ICS fetch returned HTML on first attempt — retrying with browser headers..."
        );
        try {
          ...
          const body2 =
            browserRes.data && typeof browserRes.data === "string"
              ? browserRes.data
              : "";
          if (
            !/<html|<!doctype|<body/i.test(body2) &&
            /BEGIN:VEVENT/i.test(body2)
          ) {
            ...
            for (const k of Object.keys(parsed2)) {
              ...
              if (item && item.type === "VEVENT") {
                events2.push({
                  id: item.uid || item.sequence || k,
                  summary: item.summary,
                  start: {
                    dateTime: item.start ? item.start.toISOString() : null,
                  },
                  end: { dateTime: item.end ? item.end.toISOString() : null },
                  raw: item,
                });
              }
            }
            ...
            fs.writeFileSync(
              OUT_PATH,
              JSON.stringify(
                { fetchedAt: new Date().toISOString(), data: out2 },
                null,
                2
              ),
              "utf8"
          ...
          console.error(
            "Retry with browser headers failed:",
            err2.message || err2
          );
        }
        ...
        const errOut = {
          fetchedAt: new Date().toISOString(),
          error:
            "ICS fetch returned HTML (likely requires auth or is not public)",
          rawPreview: preview,
        };
        fs.mkdirSync(path.dirname(OUT_PATH), { recursive: true });
        fs.writeFileSync(OUT_PATH, JSON.stringify(errOut, null, 2), "utf8");
        console.error(
          "ICS URL returned HTML — feed may require authentication or is invalid. Stored rawPreview in data/events.json"
        );
        return;
      }
      ...
      for (const k of Object.keys(parsed)) {
        ...
        if (item && item.type === "VEVENT") {
          events.push({
            id: item.uid || item.sequence || k,
            summary: item.summary,
            start: { dateTime: item.start ? item.start.toISOString() : null },
            end: { dateTime: item.end ? item.end.toISOString() : null },
            raw: item,
          });
        }
      ...
      const out = [
        { calendarId: "ical-public", summary: "Public ICS", events },
      ];
      fs.mkdirSync(path.dirname(OUT_PATH), { recursive: true });
      fs.writeFileSync(
        OUT_PATH,
        JSON.stringify(
          { fetchedAt: new Date().toISOString(), data: out },
          null,
          2
        ),
        "utf8"
