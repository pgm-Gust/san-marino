import { supabaseService as supabase } from "./serviceClient";

// Dit bestand is server-only (gebruikt door API routes) en gebruikt de
// service_role key. De routes die deze functies aanroepen dwingen zelf al
// autorisatie af (admin-sessiecheck of een reeds gevalideerde boekingsflow).
...
/**
 * Check of een datumbereik overlapt met een bestaande blokkering, met dezelfde
 * halfopen-interval semantiek als de rest van de app (checkout-dag zelf is vrij,
 * zodat same-day turnover blijft werken). Gebruikt voor de server-side
 * boekingscontrole om dubbele boekingen te voorkomen.
 * @param {number|string} apartmentId
 * @param {string} startDate - Aankomstdatum (YYYY-MM-DD)
 * @param {string} endDate - Vertrekdatum (YYYY-MM-DD)
 */
export async function hasOverlappingBlockedDates(
  apartmentId,
  startDate,
  endDate
) {
  try {
    ...
    if (error) {
      console.error("Error checking overlapping blocked dates:", error);
      throw error;
    }

    return Boolean(data && data.length > 0);
  } catch (error) {
    console.error("Error in hasOverlappingBlockedDates:", error);
    throw error;
  }
}
