 ...
 */
export async function fetchMinimumNights(apartmentId = null, startDate = null, endDate = null) {
  try {
    let query = supabase
      .from("minimum_nights")
      .select("*")
 ...
 */
export async function addMinimumNight(apartmentId, startDate, endDate, minNights, reason = null) {
  try {
    const { data, error } = await supabase
      .from("minimum_nights")
      .insert([
